...
export class CartService {
  ...
  postNewItem(item: Item): Observable<any> {
    return this.http.post("http://localhost:5000/people", item);
  }

  putItemChanges(item: Item): Observable<any> {
    return this.http.put(`http://localhost:5000/people/${item.id}`, item);
  }
